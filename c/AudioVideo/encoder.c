...
void getFrame(AVCodecContext *c, AVFrame *frame);
AVCodecContext* initialize_codec_context(AVCodecContext *codec_ctx);

int main(int argc, char *argv[]) {
	int return_code;

	OutputStream *stream = {NULL}; //NULL
	...
	codec_ctx = initialize_codec_context(codec_ctx);
	if(!codec_ctx) {
		fprintf(stderr, "FAIL LOSER\n");
		exit(1);
	}
	...
	frame->height = codec_ctx->height;
    	...
    	return_code = av_frame_get_buffer(frame, 32);
    	if (return_code < 0) {
	...
	// Capture Frames to be written 
		...
		getFrame(codec_ctx, frame);
		frame->pts = i;

		encodeFrame(codec_ctx, frame, pkt, file_index);
	}

	//Finalize Binary File with Footer (EOF_parameters)
    	encodeFrame(codec_ctx, NULL, pkt, file_index);//Flushes the buffer
